 ...
 */
int bang(int x) {
  /* return 1 when only x = 0
     x = 0 => (x - 1 < 0 and ~x < 0)
  */
  return (((x + ~0) >> 31) & (~x >> 31) & 1);
}
/* 
...
int fitsBits(int x, int n) {
  /* fits n bits 
     when 32-n+1 first bits are indentical (0 either or 1)
  */
...
int isLessOrEqual(int x, int y) {
   /* 2 cases: different sign => true when x is nega
               same sign => true when x - y <= 0
   */
   int sign_x, sign_y, dif_sign, less1, k;
   sign_x = (x >> 31) & 1; /* return 1 when negative*/
   sign_y = (y >> 31) & 1;
   dif_sign = sign_x ^ sign_y; /* 1 => different sign */
   less1 = dif_sign & sign_x;
   k = x + (~y + 1);
   return less1 | ((!!(k >> 31 | !k)) & !dif_sign);
}
/*
...
unsigned float_abs(unsigned uf) {
  unsigned mask = 0x7FFFFFFF;	         // 01111111...11 => set sign bit to 0
  unsigned minNaN = 0x7F800001;          // signaling NaN
